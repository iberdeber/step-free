  ...
  useEffect(() => {
    ...
    map.current.on("click", async (e) => {
      ...
      if (lng < minLng || lat < minLat || lng > maxLng || lat > maxLat){
        alert('Please select a location inside of New York City Region');
        return;
      }
      ...
      if (query) {
        const res = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
            query
          )}.json?access_token=${accessToken}&bbox=${NYC_BBOX.join(',')}`
        );
        const data = await res.json();
        ...
        if (first && region?.short_code === 'US-NY') {
          ...
          alert(`Could not find ${role} location in New York City Region`);
          return;
        }
      }
    }
