...
export default function SearchBar({ onSearchSubmit }: Props) {
  ...
  const saveAddressToHistory = async (
    address: string,
    type: "start" | "end"
  ) => {
    ...
    await supabase
      .from("address_history")
      .insert([{ user_id: userId, address, type }]);
  ...
  const fetchSuggestions = async (query: string, field: "start" | "end") => {
    ...
    const res = await fetch(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
        query
      )}.json?proximity=ip&autocomplete=true&bbox=-74.25909,40.477399,-73.700181,40.917577&access_token=${accessToken}`
    );
  ...
  const handleSubmit = async () => {
    ...
    if (startQuery.trim())
      await saveAddressToHistory(startQuery.trim(), "start");
    if (endQuery.trim()) await saveAddressToHistory(endQuery.trim(), "end");
    onSearchSubmit(values);
  };

  return (
    <div className="search-bar absolute top-10 left-4 z-50 bg-white dark:bg-black bg-opacity-10 border-white border-opacity-20 rounded-xl p-2 shadow-lg w-[320px] space-y-3">
